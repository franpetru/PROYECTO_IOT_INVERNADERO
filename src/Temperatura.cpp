...
void connect()
{
    ...
    Serial.println("\nconnected!");
    client.publish("Humedad");      // para usar en el modulo temperatura
    client.onMessage(messageReceived);  // lllama la funcion del mensage que recibe del node red .
    client.subscribe("Riego");    //para usar en el modulo de ventilacion 
...
int Message_Publish()
{
    ...
    int t1 = map(analogRead(34), 0, 4095, 0, 100); // ver como lee el sensor el pin MAUROOOO jajaja
    return (t1);
}
...
void setup()
{
    ...
    pinMode(34, INPUT);  //hum
    pinMode(26, OUTPUT); // riego
...
void loop()
{
    ...
    {
        lastTime = millis();
        client.publish("Humedad", String(Message_Publish()));
    }
